...
| --- | --- | --- | --- | --- |
| Agent Session Bridge | feature branch final commit; `atif 1.7.0` | `python -m interoperability.reproduce ...` then `python -m interoperability.verifier ...` | Five steps, two calls, two correlated results, source step timestamps, and redaction passed; one unsupported block and two unsupported records were accounted for. | ASB self-validation |
